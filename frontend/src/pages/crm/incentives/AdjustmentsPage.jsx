...
const StatusChip = ({ value = "Pending" }) => {
  ...
  return (
    <span className={`px-2 py-0.5 rounded-full text-xs ${map[value] || "bg-white/10 text-slate-300"}`}>
      {value}
    </span>
  );
};
...
export function AdjustmentsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((r) =>
      [r.employee, r.plan, r.type, r.reason, r.status].some((v) =>
  ...
  const canSubmit =
    form.date && form.employee.trim().length >= 2 && Number(form.amount) > 0;
          ...
          <tbody className="[&>tr]:border-t [&>tr]:border-white/10">
            ...
            {filtered.map((r) => (
              <tr key={r.id} className="hover:bg-white/5 transition-colors">
                ...
                <td className="px-3 py-3"><StatusChip value={r.status} /></td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          <label className="grid gap-1 md:col-span-2">
            ...
            <textarea
              className="min-h-[90px] rounded-lg border bg-background px-3 py-2"
              value={form.reason}
          ...
          {!canSubmit && (
            <div className="md:col-span-2 text-xs text-red-400">
              Enter at least: <b>Date</b>, <b>Employee</b>, <b>Amount</b>.
            </div>
          )}
        </div>
      </Modal>
