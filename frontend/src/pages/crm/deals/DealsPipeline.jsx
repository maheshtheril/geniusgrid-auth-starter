...
function Column({ id, title, count, total, children }){
  ...
  return (
    <div
      ref={setNodeRef}
      className={`flex-1 min-w-[260px] rounded-2xl border bg-card/60 backdrop-blur p-3 shadow-sm ${
...
function DealCard({ deal, onOpen }){
  ...
  return (
    <div
      ...
      {...listeners}
      {...attributes}
      className={`rounded-xl border bg-background p-3 shadow-sm cursor-grab active:cursor-grabbing hover:shadow transition ${
    ...
    >
      ...
      <div className="text-xs text-muted-foreground mt-0.5">Next: {deal.next_step || "—"}</div>
    </div>
  );
}
...
export default function DealsPipeline(){
  ...
  // Get the opener function from DealsLayout (where the drawer lives)
  ...
  // Refresh when a deal is created elsewhere (DealsLayout drawer)
  useEffect(() => {
  ...
  const onDragEnd = async (event) => {
    ...
    try { await moveDealToStage(id, overCol); } catch (e) { console.warn(e); }
  };

  return (
    <div>
      {/* Toolbar: New Deal now opens the main drawer in DealsLayout */}
            ...
            >
              {(byStage[s.id] || []).map(d => (
                <DealCard
                  key={d.id}
                  deal={d}
                  onOpen={(deal)=> setDrawer({ open: true, deal })}
                />
              ))}
            </Column>
          ))}
