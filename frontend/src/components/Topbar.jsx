...
import {
  ...
  Search, Sun, Moon, MonitorSmartphone, CheckCircle2
} from "lucide-react";
import { Link, useLocation, useNavigate } from "react-router-dom";
import { useEnv } from "@/store/useEnv";
import { readMode } from "@/theme/mode"; // only readMode needed
...
// ---- Local theme helpers (no external dependency) ----
const THEMES = ["dark", "light", "night"]; // cycle order
...
function applyDomModeInline(mode) {
  ...
  // daisyUI / CSS variables hook
  root.setAttribute("data-theme", mode);
  document.body?.setAttribute("data-theme", mode);
  // Tailwind dark: + native color-scheme
...
function ThemeIcon({ value }) {
  ...
  if (value === "dark" || value === "night") return <Moon className="w-4 h-4" />;
...
export default function Topbar({ collapsed, onToggleCollapse, onBurger }) {
  ...
  const [open, setOpen] = React.useState({
    module:false, create:false, notif:false, help:false, user:false, company:false
  });
  ...
  // effects
  React.useEffect(() => {
    // Initial apply (in case boot didn't)
    applyDomModeInline(theme);

    // load modules & quick actions
    fetch(api("/tenant/menus?level=top"), { credentials: "include" })
      .then(r => r.ok ? r.json() : [])
      .then(setModules)
      .catch(() => setModules([]));

    fetch(api("/tenant/quick-actions"), { credentials: "include" })
      .then(r => r.ok ? r.json() : [])
    ...
    (async () => {
      try {
        let res = await fetch(api("/notifications/summary"), { credentials: "include" });
        let unread = 0, items = [];
        if (res.ok) {
          ...
          res = await fetch(api("/notifications?limit=10"), { credentials: "include" });
          if (res.ok) {
            const d = await res.json();
            unread = Number(d?.unread_count || 0);
            items = Array.isArray(d?.items) ? d.items : [];
  ...
  async function markAllRead() {
    try {
      ...
      if (r.ok) setNotif(n => ({
        ...n,
        unread: 0,
        items: n.items.map(it => ({ ...it, read_at: it.read_at || new Date().toISOString() }))
      }));
    } catch {}
  }
  ...
  return (
    <header className="sticky top-0 z-40 bg-base-100 border-b border-base-300">
      <div className="h-14 flex items-center gap-2 px-2 sm:px-4">
        {/* Left: burger + collapse + Company + FY + Modules */}
        <div className="flex items-center gap-1">
          ...
          <div className="relative">
            <button
              ...
              aria-haspopup="menu" aria-expanded={open.company}
          ...
          {modules.length > 0 && (
            <div className="relative">
              <button
                ...
                aria-haspopup="menu" aria-expanded={open.module}
              >
        ...
        {/* Right: Create, Notifications, Help, Theme, User */}
        <div className="flex items-center gap-1">
          {quickActions.length > 0 && (
            <div className="relative">
              <button
                ...
                aria-haspopup="menu" aria-expanded={open.create}
                title="Quick create"
              >
          ...
          <div className="relative">
            <button
              ...
              aria-haspopup="menu" aria-expanded={open.notif}
            >
          ...
          <div className="relative">
            <button
              ...
              aria-haspopup="menu" aria-expanded={open.help}
              title="Help & docs"
            >
          ...
          <div className="relative">
            <button
              ...
              aria-haspopup="menu" aria-expanded={open.user}
              title="Account"
            >
