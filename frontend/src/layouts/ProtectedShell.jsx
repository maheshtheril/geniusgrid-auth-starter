...
export default function ProtectedShell({ title, primaryAction }) {
  ...
  const [collapsed, setCollapsed] = React.useState(false); // desktop mini sidebar (w-16)
  const [cmdOpen, setCmdOpen] = React.useState(false); // command palette
  const [theme, setTheme] = React.useState(getInitialTheme());

  // Persist + apply theme (daisyUI expects data-theme on <html>)
  React.useEffect(() => {
  ...
  // Global shortcut: ⌘K / Ctrl+K opens command palette
  React.useEffect(() => {
    const onKey = (e) => {
  ...
  const crumbs = React.useMemo(() => {
    ...
    const pretty = (s) => {
      const map = {
        admin: "Admin", crm: "CRM", leads: "Leads",
        companies: "Companies", contacts: "Contacts", deals: "Deals",
        reports: "Reports", org: "Organization", branding: "Branding",
        localization: "Localization", calendars: "Calendars", api: "API",
      };
  ...
  return (
    // CSS var (--sbw) keeps header and content perfectly aligned with the sidebar
    <div
    ...
    >
      ...
      {/* World-class Topbar (aligned to sidebar via md:pl-[var(--sbw)]) */}
      <header
        role="banner"
      ...
      >
        <div className="h-14 px-2 sm:px-3 flex items-center gap-2">
          ...
          {primaryAction && (
            <button className="btn btn-primary btn-sm md:btn gap-2" onClick={primaryAction.onClick}>
              <svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" strokeWidth="2">
      ...
      {/* Main content aligned with sidebar via md:pl-[var(--sbw)] */}
        ...
        >
          <div className="w-full max-w-2xl rounded-2xl bg-base-100 border border-base-300 shadow-xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-3 text-sm opacity-70">
              Type to search… (wire this to your menu API or AppSidebar data when ready)
            </div>
          </div>
        </div>
      )}
