// backend/src/api/admin/users.routes.js
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (q) {
        ...
        where.push(`(LOWER(u.name) LIKE $${idx} OR LOWER(u.email) LIKE $${idx} OR COALESCE(u.phone,'') LIKE $${idx})`);
        idx++;
      }
      if (role_id) {
        params.push(role_id);
        where.push(`EXISTS (SELECT 1 FROM public.user_roles ur WHERE ur.tenant_id=u.tenant_id AND ur.user_id=u.id AND ur.role_id=$${idx})`);
        idx++;
      }
      if (company_id) {
        params.push(company_id);
        where.push(`EXISTS (SELECT 1 FROM public.res_user_companies uc WHERE uc.tenant_id=u.tenant_id AND uc.user_id=u.id AND uc.company_id=$${idx})`);
        idx++;
      }
      if (status) {
        if (status === "Active") where.push(`u.is_active = true AND COALESCE(u.is_locked,false) = false`);
        else if (status === "Suspended") where.push(`COALESCE(u.is_locked,false) = true`);
        else if (status === "Invited") where.push(`u.email_verified = false`);
      }

      const whereSql = where.length ? `WHERE ${where.join(" AND ")}` : "";

      // last_active from user_sessions
      const base = `
        WITH base AS (
          SELECT u.*,
                 (SELECT max(s.created_at)
                    FROM public.user_sessions s
                   WHERE s.tenant_id = u.tenant_id AND s.user_id = u.id) AS last_active
 ...
 * Notes:
 *  - If you hash password in DB trigger, we pass a placeholder to `password`
 * Needs: user_manage
 * =================================================================== */
router.post(
  "/users",
 ...
 * Needs: user_manage (or role_manage if you prefer)
 * =================================================================== */
router.get(
  "/roles",
  requirePermission("user_manage"),
