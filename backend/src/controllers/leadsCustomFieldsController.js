...
/** READ-ONLY: return active version id if present, else null */
export async function findActiveLeadsFormVersionId(tenantId) {
  const client = await pool.connect();
  try {
    await setTenantScope(client, tenantId);
    const { rows } = await client.query(
      ...
      select v.id
      from custom_forms f
      join custom_form_versions v
        on v.tenant_id = f.tenant_id and v.form_id = f.id and v.status = 'active'
      where f.tenant_id = $1 and f.module_name = 'crm' and f.code = 'leads'
...
/** CREATE-IF-MISSING: ensure form + active version; return version id */
export async function getActiveLeadsFormVersionId(tenantId) {
  const client = await pool.connect();
  try {
    await setTenantScope(client, tenantId);

    // ensure form row exists; ON CONFLICT ... RETURNING id always yields an id
    ...
    // ensure active version exists (CTE approach — NO "select from (insert ...)")
...
/** Upsert a field (⚠️ your table has no updated_at column, so don't set it) */
...
/** Upsert values for a lead (values table DOES have updated_at) */
    ...
    for (const { code, value } of values) {
      ...
      switch ((f.field_type || "").toLowerCase()) {
        ...
        case "date":
          ...
          break;
        case "file":
          cols.file = value || null; // file_id uuid
          break;
        case "select":
        case "text":
        default:
